...
log_prompt "INFO" && read -p "Choisissez sur quelle partition sera installé le système (ex: 3 pour /dev/sda3 ==> ${MOUNT_POINT}) : " root_partition_num && echo ""
...
if [[ "$mount_more" == "y" ]]; then
  ...
  for partition in "${partitions[@]}"; do
    ...
    while true; do
      log_prompt "INFO" && read -p "Nommer le point de montage de la partition ${partition} (ex. ${partition} $MOUNT_POINT/ ==> efi) : " partition_name && echo ""
